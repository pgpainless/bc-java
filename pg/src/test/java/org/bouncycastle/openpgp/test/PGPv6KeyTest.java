    ...
    extends SimpleTest
{
    ...
    private void test() throws IOException, PGPException {
        ByteArrayInputStream bIn = new ByteArrayInputStream(ARMORED_KEY.getBytes());
        ArmoredInputStream armorIn = new ArmoredInputStream(bIn);
        BCPGInputStream bcIn = new BCPGInputStream(armorIn);

        PGPSecretKeyRing secretKeys = new PGPSecretKeyRing(bcIn, new BcKeyFingerprintCalculator());
        PGPPrivateKey unencryptedPriv = secretKeys.getSecretKey().extractPrivateKey(null);

        PGPSecretKeyRing encrypted = PGPSecretKeyRing.copyWithNewPassword(secretKeys, null,
                new BcPBESecretKeyEncryptorBuilder(
                        SymmetricKeyAlgorithmTags.AES_256, AEADAlgorithmTags.OCB, new S2K.Argon2Params()
                ).build("sw0rdf1sh".toCharArray()));

        ByteArrayOutputStream bOut = new ByteArrayOutputStream();
        ArmoredOutputStream armorOut = new ArmoredOutputStream(bOut);
        encrypted.encode(armorOut);
        armorOut.close();
        ...
        PGPSecretKeyRing parsed = new PGPSecretKeyRing(bcpgInputStream, new BcKeyFingerprintCalculator());
        PGPPrivateKey privateKey = parsed.getSecretKey().extractPrivateKey(
                new BcPBESecretKeyDecryptorBuilder(new BcPGPDigestCalculatorProvider()).build("sw0rdf1sh".toCharArray()));

        isTrue(Arrays.areEqual(unencryptedPriv.getPrivateKeyDataPacket().getEncoded(), privateKey.getPrivateKeyDataPacket().getEncoded()));
    }
